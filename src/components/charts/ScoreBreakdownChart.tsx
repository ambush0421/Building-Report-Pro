...
const ScoreBreakdownChart: React.FC<ScoreBreakdownChartProps> = ({ breakdown }) => {
  const data = [
    { name: '가성비', score: Math.max(0, breakdown.costScore), color: '#000000' },
    { name: '공간성', score: Math.max(0, breakdown.areaScore), color: '#333333' },
    { name: '편의성', score: Math.max(0, breakdown.parkingScore), color: '#666666' },
    { name: '신축도', score: Math.max(0, breakdown.modernityScore), color: '#999999' },
  ];
