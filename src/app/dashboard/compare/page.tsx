...
export default function ComparePage() {
  ...
  const [type, setType] = useState<'LEASE' | 'PURCHASE' | 'INVEST'>('LEASE');
  ...
  const handleCompare = async (updatedWeights = weights) => {
    setLoading(true);
    try {
      const response = await fetch('/api/building-report-v2', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type, items, currentCost, weights: updatedWeights })
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error);
      setComparisonData(data);
      setMobileIdx(data.recommendation.bestBuildingIndex);
    } catch (err: any) {
      alert(err.message || '비교 보고서 생성 중 오류가 발생했습니다.');
    } finally {
      setLoading(false);
    }
  };
